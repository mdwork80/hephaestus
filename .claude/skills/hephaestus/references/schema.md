...
|---|---|---|---|
| `project_name` | str | 1–120 chars | ask |
| `project_slug` | str | `^[a-z][a-z0-9]*(-[a-z0-9]+)*$`, 2–64 chars | derived: name → lowercase, spaces/underscores → hyphens |
| `description` | str | 10–280 chars, one sentence | ask |
| `owner` | str | 1–120 chars, accountable individual | ask |
...
| `network_isolation` | enum | `public` `private` | `public`; only ask when target is azure/hybrid |
| `primary_language` | str | **free-form** lowercase identifier — any programming or scripting language (`python`, `rust`, `go`, `typescript`, `dotnet`, `powershell`, `bash`, `ruby`, …) | ask |
| `runtime_pattern` | enum | `cli` `web_app` `api_service` `background_worker` `scheduled_job` `agent_pipeline` `library` | `cli` |
...
## Cross-field rules (all 11 — enforce at intake AND in the generated validator)
...
8. `auth_model: none` ⇒ requires `runtime_pattern` in (`cli`, `library`) OR `deployment_target: local_only`.
9. `ai_tooling: agentic` ⇒ `threat_model_required: true`.
10. `cors_origins` non-empty ⇒ `runtime_pattern` in (`web_app`, `api_service`).
11. `network_isolation: private` ⇒ `deployment_target` in (`azure`, `hybrid`).

At intake: rules 4 and 9 are auto-corrections (set the bool true, inform the user). All others are re-ask-with-explanation.
...
| Option | Values | Default | When to ask |
|---|---|---|---|
...
| `ssh_scaffold` | `none` / `client` / `server` / `both` | `none` | only if the user asks for SSH functionality; generalization of the old Rust-only `rust_ssh`. Server mode is network-exposed attack surface: key-based auth only (ed25519, no passwords), and recommend `threat_model_required: true` + a real `auth_model` |

Derived (never asked): package/module/crate identifier = `project_slug` with hyphens → underscores.
