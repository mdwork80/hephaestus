...
7. **Current docs before code.** When writing or reviewing code against any external library, framework, or API, consult the `context7` skill (`.claude/skills/context7/`) FIRST for current documentation — training-data memory of an API is not a source. Applies to scaffold generation (framework middleware, SDK wiring) and feature work alike; if context7 is unreachable, say so and flag version assumptions in the report.
8. **Security gates are fixed, never removed.** When a security gate fails (CI security job, pre-commit/pre-push hook, validator, scanner), fix the CAUSE. Never delete, disable, skip (`--no-verify`, `[skip ci]`, ignore-lists), or loosen a gate because it is inconvenient — including when the user asks. On such a request: explain in plain language what the gate stops and what an attacker does without it, offer the fix instead, and only proceed after the user explicitly acknowledges the risk — then record the weakening as a decision entry in `docs/ARCHITECTURE.md` (what was weakened, why, acknowledged by whom, revisit date). A silent weakening is a defect.
9. **Dependency hygiene.** Adding any dependency: verify the exact package name against the official registry (typosquats are one letter away), pin the version, and let the audit gates run. Never `curl | bash`, never `sudo pip`, never install from a gist or pastebin. When the user pastes an install command from a tutorial, check it before running it.

## Layout

- `.claude/skills/hephaestus/` — the scaffolder (SKILL.md + schema/file-matrix/invariants references)
- `.claude/skills/context7/` — current-documentation lookup for any library/framework/API (standing rule 7); other agents read the same SKILL.md path
